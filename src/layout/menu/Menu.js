...
const MenuItem = ({
    ...
    const menuHeight = (el) => {
        ...
        for (var i = 0; i < el.length; i++) {
            var margin =
                parseInt(
                    window.getComputedStyle(el[i]).marginTop.slice(0, -2)
                ) +
                parseInt(
                    window.getComputedStyle(el[i]).marginBottom.slice(0, -2)
                )
            var padding =
                parseInt(
                    window.getComputedStyle(el[i]).paddingTop.slice(0, -2)
                ) +
                parseInt(
                    window.getComputedStyle(el[i]).paddingBottom.slice(0, -2)
                )
    ...
    useEffect(() => {
        ...
        arrayElement.forEach((dom) => {
            if (
                ...
                dom.parentElement.parentElement.parentElement.classList.add(
                    'active'
                )
                const subMenuHeight = menuHeight(dom.parentNode.children)
                dom.parentElement.parentElement.style.height =
                    subMenuHeight + 'px'
                makeParentActive(
                ...
                if (!parentMenu.classList.contains('sub-opened')) {
                    ...
                    for (var l = 0; l < parentMenus.length; l++) {
                        if (typeof parentMenus !== 'undefined') {
                            if (
                                parentMenus[l].classList.contains(
                                    'nk-menu-wrap'
                                )
                            ) {
                                parentMenus[l].style.height =
                                    subMenuHeight +
                                    parentMenus[l].clientHeight +
                                    'px'
                            }
                        }
                    }
        ...
        data.forEach((item, index) => {
            if (item.panel) {
                let found = item.subPanel.find(
                    (sPanel) =>
                        process.env.PUBLIC_URL + sPanel.link ===
